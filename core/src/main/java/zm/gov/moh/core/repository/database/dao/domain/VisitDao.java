...
public interface VisitDao extends Synchronizable<Long> {
    ...
    @Query("SELECT visit_id FROM (SELECT * FROM visit WHERE visit_id NOT IN (SELECT DISTINCT entity_id FROM " +
            "entity_metadata WHERE entity_type_id + :entityTypeId AND remote_status_code + :remoteStatus AND entity_metadata.entity_id + visit.visit_id)) " +
            "WHERE visit_id >= :offsetId Limit 2")
